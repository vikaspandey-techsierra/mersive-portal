...
export default function CollaborationUsage({
  ...
  const isAllZeroData = useMemo(() => {
    return chartData.every(
      (d) => (d.avgConnections ?? 0) === 0 && (d.avgPosts ?? 0) === 0,
    );
  }, [chartData]);
  ...
  return (
    <div className="mb-8">
      ...
      <div className="bg-white rounded-xl p-5 py-5 pb-4 border border-gray-200 ">
        {isAllZeroData || chartData.length === 0 ? (
          <EmptyState
            title="No data for this date range"
            description="Collaboration metrics appear when users connect and share content in meetings"
            icon={peopleIcon}
          />
        ) : (
