...
const StatCards = ({ stats }: { stats: DashboardStats }) => {
  const cards: {
    ...
    },
    {
      ...
      value: stats.avgMeetingLengthMin ? (
        stats.avgMeetingLengthMin
      ) : (
        <div className="w-6 h-1.5 bg-[#090814] my-5" />
      ),
    },
    {
      icon: TrendingUpIcon,
  ...
  return (
    <div className="flex max-lg:grid grid-cols-2 justify-around gap-6 overflow-hidden text-[#090814] font-poppins px-8">
      {cards.map((card, i) => (
        <div
          key={i}
          className="flex flex-col justify-center gap-3 px-4 py-6 border border-gray-200 rounded-lg w-full h-40"
        >
